...
import { loadProfiles, saveProfiles, getCurrentProfile } from './profiles.js'; // Přidáno
...
/**
 * Aktualizuje statistiky aktuálního profilu.
 * @param {string} key - Klíč statistiky, která se aktualizuje.
 * @param {number} value - Hodnota, kterou se statistika zvyšuje.
 */
function updateProfileStatistics(key, value = 1) {
    const currentProfile = getCurrentProfile();
    if (!currentProfile) return;

    if (currentProfile.statistics.hasOwnProperty(key)) {
        currentProfile.statistics[key] += value;
    } else {
        console.warn(`Statistika "${key}" není definována pro profil "${currentProfile.username}".`);
    }

    // Uložit změny zpět do localStorage
    const profiles = loadProfiles();
    const profileIndex = profiles.findIndex(profile => profile.username === currentProfile.username);
    if (profileIndex !== -1) {
...
/**
 * Zpracuje odpověď uživatele a aktualizuje skóre a statistiky
  ...
  function handleAnswer(isCorrect) {
    if (currentMode === 'testing') {
        if (isCorrect) {
            ...
            updateProfileStatistics('noAnswers');
        } else {
